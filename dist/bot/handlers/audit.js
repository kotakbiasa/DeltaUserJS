...
export function registerAuditHandlers(bot) {
    ...
    bot.callbackQuery('audit:export:json', async (ctx) => {
        ...
        try {
            ...
            await ctx.replyWithDocument(new InputFile(fileBuffer, `audit-logs-${new Date().toISOString().split('T')[0]}.json`), { caption: `📋 Audit Log Export (${logs.length} entri)` });
        }
        catch (err) {
    ...
    bot.callbackQuery('audit:export:csv', async (ctx) => {
        ...
        try {
            ...
            await ctx.replyWithDocument(new InputFile(csvBuffer, `audit-logs-${new Date().toISOString().split('T')[0]}.csv`), { caption: `📋 Audit Log Export CSV (${logs.length} entri)` });
        }
        catch (err) {
