            ...
            try {
                ...
                if (cmd === 'anichar') {
                    ...
                    if (media.title.native)
                        text += ` (<code>${media.title.native}</code>)\n`;
                    text += `<blockquote>`;
                    if (media.startDate?.year)
                        text += `📅 <b>Tahun:</b> <code>${media.startDate.year}</code>\n`;
                    if (media.status)
                        text += `📡 <b>Status:</b> <code>${media.status}</code>\n`;
                    if (media.averageScore)
                        text += `⭐️ <b>Skor:</b> <code>${media.averageScore}</code>\n`;
                    if (media.genres?.length)
                        text += `🎭 <b>Genre:</b> ${media.genres.join(', ')}\n`;
                    text += `</blockquote>`;
