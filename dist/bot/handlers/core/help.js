...
import { escapeHtml } from '../../keyboards/dashboard.js';
...
export function registerInlineHelpHandlers(bot) {
    ...
    bot.callbackQuery('help:close', async (ctx) => {
        ...
        if (ctx.inlineMessageId) {
            await ctx.api.editMessageTextInline(ctx.inlineMessageId, { text: 'Menu bantuan ditutup.' }, { reply_markup: { inline_keyboard: [] } }).catch(() => { });
        }
        else {
            try {
                await ctx.deleteMessage();
