...
export function registerFederationHandlers(bot) {
    ...
    bot.command('newfed', async (ctx) => {
        if (!isOwner(ctx.from.id))
            return replyRich(ctx, '❌ Hanya owner bot yang bisa membuat Federasi utama.');
        const fedName = ctx.match.trim();
        if (!fedName)
            return replyRich(ctx, '❌ Format: `/newfed <Nama Fed>`', { markdown: true });
    ...
    bot.command('joinfed', modCheck, async (ctx) => {
        ...
        if (!fedId)
            return replyRich(ctx, '❌ Format: `/joinfed <Fed ID>`', { markdown: true });
        const fed = getFederation(fedId);
        if (!fed)
            return replyRich(ctx, '❌ Federasi tidak ditemukan.');
    ...
    bot.command('unfban', modCheck, async (ctx) => {
        ...
        const config = await getGroupConfig(chatId);
        if (!config.linked_fed)
            return replyRich(ctx, '❌ Grup tidak terhubung ke federasi.');
        const fed = getFederation(config.linked_fed);
        if (!fed)
            return replyRich(ctx, '❌ Federasi tidak ditemukan.');
        if (!fed.admins.includes(ctx.from.id))
            return replyRich(ctx, '❌ Anda bukan admin federasi.');
        let targetId = null;
        if (ctx.message.reply_to_message) {
            targetId = ctx.message.reply_to_message.from.id;
        }
        ...
        if (!targetId || isNaN(targetId))
            return replyRich(ctx, '❌ Reply pesan atau berikan ID.');
