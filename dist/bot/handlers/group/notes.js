...
export function registerNotesHandlers(bot) {
    ...
    bot.command('save', async (ctx) => {
        ...
        if (parts.length < 2) {
            return replyRich(ctx, quote(`${b('❌ FORMAT SALAH')}\nGunakan: ${code('/save namacatatan [isi]')} ` +
    ...
    bot.command('notes', async (ctx) => {
        ...
        try {
            ...
            await replyRich(ctx, quote(`${b('📝 Daftar Catatan Grup')}\n\n${lines}\n\nKetik nama catatan untuk melihat isinya.`, { expandable: notes.length > 8 }));
        }
        catch (err) {
