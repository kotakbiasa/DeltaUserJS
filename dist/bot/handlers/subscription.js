...
import { getActivePlans, getPlan, getUserSubscription, getUserPayments, initTrialSubscription, cancelAutoRenew, createPayment, getSubscriptionStats, updatePaymentStatus, } from '../../services/SubscriptionService.js';
...
export function registerSubscriptionHandlers(bot) {
    ...
    bot.callbackQuery(/^sub:buy:(.+)$/, async (ctx) => {
        ...
        try {
            ...
            await ctx.reply(`<b>💳 PEMBAYARAN ${plan.name.toUpperCase()}</b>\n\n` +
                `📦 Paket: ${plan.name}\n` +
    ...
    bot.callbackQuery('sub:renew', async (ctx) => {
        ...
        try {
            ...
            await ctx.reply(`<b>🔄 PERPANJANGAN ${plan.name.toUpperCase()}</b>\n\n` +
                `💰 Harga: ${new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(plan.price)}\n` +
                `⏰ Expired: 30 menit\n\n` +
    ...
    bot.callbackQuery('sub:help', async (ctx) => {
        await ctx.answerCallbackQuery();
        await ctx.reply(`<b>❓ BANTUAN LANGGANAN</b>\n\n` +
