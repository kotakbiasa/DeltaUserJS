...
function richKpis(caption, items) {
    const cells = items.map(([label, value]) => `<td align="center"><b>${escapeRichHtml(label)}</b><br>${escapeRichHtml(stripHtml(value))}</td>`).join('');
...
export async function sendRichPanel(ctx, html, replyMarkup, fallbackText) {
    try {
        await ctx.replyWithRichMessage({ html }, { reply_markup: replyMarkup });
        try {
...
export const ubotSettingsMenu = new Menu('ubot-settings-menu')
    ...
    await ctx.answerCallbackQuery();
    await editRich(ctx, `🔺 <b>D E L T A   U B O T   J S</b> 🔺\n` +
        `───────────────────────\n` +
        `⚠️ <b>KONFIRMASI PENGHAPUSAN SESI</b>\n\n` +
        `Tindakan ini akan mematikan userbot dan menghapus session string dari database.\n\n` +
        `Jika hanya ingin berhenti sementara, gunakan tombol <b>Matikan Userbot</b>, bukan hapus sesi.`, { reply_markup: dangerDeleteMenu });
}).row()
...
export const ubotMainMenu = new Menu('ubot-main-menu')
    ...
    await ctx.replyWithRichMessage({ html: buildHelpMenuRichHtml(dbSession, 1, totalPages) }, { reply_markup: helpKeyboard(1, 'main') });
    try {
        await ctx.deleteMessage();
    }
    catch (_) { }
...
export const adminManageUserMenu = new Menu('admin-manage-user-menu')
    ...
    // Toggle button
    range.text(isRunning ? '🔌 Matikan Ubot' : '⚡ Hidupkan Ubot', async (ctx) => {
        if (isRunning) {
