import { PlanModel, PaymentModel, SubscriptionModel, AuditLogModel, DEFAULT_PLANS } from '../infrastructure/subscriptionModels.js';
...
export async function upsertPlan(data) {
    const plan = await PlanModel.findByIdAndUpdate(data._id, { ...data, updatedAt: new Date() }, { upsert: true, new: true, runValidators: true });
 ...
 */
export async function updatePaymentStatus(externalId, data) {
    const payment = await PaymentModel.findOneAndUpdate({ externalId }, {
        ...data,
        status: data.status,
        updatedAt: new Date(),
...
export async function activateSubscription(userId, planId, paymentId) {
    ...
    const graceEndDate = endDate
        ...
        : null;
    const subscription = await SubscriptionModel.findOneAndUpdate({ userId }, {
        userId,
        planId,
        paymentId,
...
export async function renewSubscription(userId, planId, paymentId) {
    ...
    const graceEndDate = endDate
        ? new Date(endDate.getTime() + GRACE_PERIOD_DAYS * 24 * 60 * 60 * 1000)
        : null;
    const updated = await SubscriptionModel.findOneAndUpdate({ userId }, {
        planId,
        paymentId,
        status: 'active',
        endDate,
 ...
 */
export async function cancelAutoRenew(userId, reason) {
    const subscription = await SubscriptionModel.findOneAndUpdate({ userId }, {
        autoRenew: false,
...
export async function checkExpiredSubscriptions() {
    ...
    for (const sub of inGrace) {
        ...
        await notifyUser(sub.userId, `⚠️ <b>Masa aktif userbot kamu sudah habis</b>\n\n` +
    ...
    for (const sub of expired) {
        ...
        await notifyUser(sub.userId, `🚫 <b>Userbot kamu dinonaktifkan</b>\n\n` +
    ...
    for (const sub of trialExpired) {
        ...
        await notifyUser(sub.userId, `⏰ <b>Trial userbot kamu berakhir</b>\n\n` +
...
export async function getSubscriptionStats() {
    const [total, active, expired, trial, grace, cancelled, revenue,] = await Promise.all([
 ...
 */
async function updateUserbotExpiration(userId, endDate) {
    await UserbotModel.updateOne({ telegram_id: userId }, { $set: { expired_at: endDate } }).catch(err => Logger.logSystem(`Failed to update userbot expiration: ${err}`, 'ERROR'));
}
/**
 * Audit log helper
 */
async function logAudit(action, resource, resourceId, before, after, actorId, userId, metadata) {
    try {
