...
export default {
    ...
    help: {
        ...
        usage: '• `.suit <batu/gunting/kertas>` — suit lawan userbot (random)\n' +
        ...
        detail: 'Kuis tebakkata: bot menampilkan definisi dari kosakata hardcoded, anggota menjawab dengan me-reply pesan kuis atau mengetik `.tebakkata <jawaban>`. Jawaban benar mendapat pesan selamat (hanya fun message, tanpa poin). State kuis disimpan per chat dan otomatis kedaluwarsa 2 menit setelah dibuat.'
    },
    async execute(client, message, _settings, _telegramId) {
        ...
        if (suitMatch) {
            ...
            if (!SUIT_EMOJI[userChoice]) {
                await message.edit({
                    text: `<blockquote>❌ <b>Format salah:</b> <code>.suit &lt;batu/gunting/kertas&gt;</code>\n` +
        ...
        if (quizMatch) {
            ...
            if (['lewat', 'skip', 'menyerah'].includes(arg.toLowerCase())) {
                ...
                quizStore.delete(String(chatId));
                await message.edit({
                    text: `🏳️ <b>Kuis dinyatakan lewat.</b>\n\n` +
