...
async function playTurn(client, chatId, state, cellIdx) {
    ...
    if (winnerOf(state.board, 'X')) {
        return endGame(client, chatId, state, 'user');
    }
    if (isFull(state.board)) {
        return endGame(client, chatId, state, 'draw');
    }
    const empties = [];
    ...
    state.board[aiIdx] = 'O';
    if (winnerOf(state.board, 'O')) {
        return endGame(client, chatId, state, 'ai');
    }
    if (isFull(state.board)) {
        return endGame(client, chatId, state, 'draw');
    }
    const statusLine = `🎯 Kamu = ${PLAYER_MARK} • Aku = ${AI_MARK}\n` +
...
export default {
    ...
    help: {
        ...
        description: 'Tic tac toe lawan userbot dengan papan emoji yang diedit langsung, plus truth or dare dari API (fallback soal bahasa Indonesia).',
        usage: '• `.xo` — mulai tic tac toe lawan userbot\n' +
        ...
        detail: 'XO: kamu main ❌, userbot balikin langkah random ⭕ di papan 3x3 yang diedit di satu pesan. Menang/seri terdeteksi otomatis. ' +
    ...
    async execute(client, message, _settings, _telegramId) {
        ...
        if (xoMatch) {
            ...
            if (arg) {
                await message.edit({
                    text: `<blockquote>❌ <b>Format salah:</b> <code>.xo</code> untuk mulai, atau <code>.xo &lt;1-9&gt;</code> untuk isi kotak.</blockquote>`,
                    parseMode: 'html'
                });
                return;
            }
        ...
        if (wtpMatch) {
            ...
            if (kind !== 'truth' && kind !== 'dare') {
                await message.edit({
                    text: `<blockquote>❌ <b>Format salah:</b> <code>.wtp truth</code> atau <code>.wtp dare</code></blockquote>`,
                    parseMode: 'html'
                });
                return;
            }
