...
async function autoReplyIncoming(client, message, chatKey, text) {
    ...
    try {
        ...
        Logger.logSystem(`kwfilter: gagal auto-reply "${matched.trigger}" di chat ${chatKey}: ${err instanceof Error ? err.message : String(err)}`, 'ERROR');
    }
}
export default {
    name: 'kwfilter',
    ...
    help: {
        ...
        usage: '• `.addfilter <trigger> <reply>` — tambah/perbarui filter (boleh juga balas pesan teks tanpa <reply> untuk memakai isi pesan itu)\n' +
            '• `.delfilter <trigger>` — hapus filter\n' +
    ...
    async execute(client, message, settings, telegramId) {
        ...
        if (addMatch) {
            ...
            if (!rest) {
                await message.edit({
                    text: `<blockquote>❌ <b>Format salah:</b> <code>.addfilter &lt;trigger&gt; &lt;reply&gt;</code>\n` +
            ...
            if (!replyText) {
                ...
                if (replied && replied.message) {
                    replyText = replied.message;
                }
            }
            if (!replyText) {
                await message.edit({
                    text: `<blockquote>❌ <b>Reply belum diisi.</b>\n` +
        ...
        if (/^\.listfilter\s*$/i.test(text)) {
            ...
            if (entries.length === 0) {
                await message.edit({
                    text: `<blockquote>📝 Belum ada filter di chat ini.\n` +
