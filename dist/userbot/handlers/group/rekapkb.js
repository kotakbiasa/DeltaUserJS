...
const ENTRY_RE = /^(.+?)\s*(?:=>|->|→|:|—|–|=|\s+-\s+)\s*([\d.,]+\s*[kmb]?)(?:\s*(?:→|->|=>|\/\/)\s*[\d.,]+\s*[kmb]?)?\s*$/i;
...
const SKIP_NAME_RE = /(total|rata|terbanyak|terbesar|pemain|entri|jumlah|selisih|average|fee|win|hasil)/i;
function badge(i) {
    return MEDALS[i] || `${i + 1}.`;
}
...
function parseRekap(text) {
    ...
    for (const rawLine of String(text || '').split(/\r?\n/)) {
        ...
        if (!name || /^\d+$/.test(name) || SKIP_NAME_RE.test(name)) {
            continue;
        }
...
async function getRepliedText(message) {
    try {
...
export default {
    ...
    help: {
        ...
        description: 'Tools rekap transaksi untuk game grup: jumlahkan list nilai per pemain dari pesan yang di-reply, lalu hitung kemenangan tiap orang setelah dipotong fee.',
        usage: '• Reply pesan berisi list, lalu ketik `.rekap` — format list: "Nama: angka" atau "Nama - angka"\n' +
    ...
    async execute(client, message, _settings, _telegramId) {
        ...
        if (/^\.rekap\s*$/i.test(text)) {
            ...
            if (!raw) {
                await message.edit({
                    text: `<blockquote>❌ <b>Reply dulu pesannya!</b>\n` +
        ...
        if (winMatch) {
            ...
            if (!feeRaw) {
                await message.edit({
                    text: `<blockquote>❌ <b>Format salah:</b> <code>.win &lt;fee%&gt;</code>\n` +
            ...
            if (!isFinite(fee) || fee <= 0 || fee >= 100) {
                await message.edit({
                    text: `<blockquote>❌ Fee harus angka antara 0-100 (boleh desimal).\n` +
            ...
            if (!raw) {
                await message.edit({
                    text: `<blockquote>❌ Reply pesan rekap (output <code>.rekap</code>) atau list <code>Nama: angka</code> untuk dihitung.</blockquote>`,
                    parseMode: 'html'
                });
                return;
            }
            ...
            await message.edit({
                text: `🏆 <b>KEMENANGAN — FEE ${formatFee(fee)}%</b>\n\n` +
