...
function extFromContentType(ctype) {
    ...
    if (ctype.includes('webp')) {
        return 'webp';
    }
    if (ctype.includes('gif')) {
        return 'gif';
    }
    if (ctype.includes('mp4')) {
        return 'mp4';
    }
    return 'jpg';
}
export default {
    ...
    help: {
        ...
        usage: '• `.meme` — meme random dari r/memes\n' +
            '• `.meme <subreddit>` — meme dari subreddit lain, contoh: `.meme dankmemes`',
        detail: 'Sumber data: meme-api.com/gimme/<subreddit> (JSON berisi url gambar, postLink, dan title). ' +
    ...
    async execute(client, message, _settings, telegramId) {
        ...
        try {
            ...
            const caption = `<blockquote>${escapeHtml(title)}</blockquote>\n\n` +
