...
async function resolvePack(client, me, kind, startPack) {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
        ...
        try {
            ...
            if (!/STICKERSET_INVALID|SHORTNAME|SHORT_NAME/i.test(errMsg(e))) {
                throw e;
            }
        }
        if (!existing) {
...
async function uploadTempAsDocument(client, filePath, attributes = null) {
    ...
    if (!doc) {
        throw new Error('Upload ke Saved Messages gagal');
    }
    try {
...
async function addStickerToPack(client, packInfo, candidate, emoji) {
    ...
    try {
        ...
        if (!candidate.fallback || !retryable) {
            throw e;
        }
    ...
    async execute(client, message, _settings, telegramId) {
        ...
        if (/^\.kang(?:\s|$)/i.test(text)) {
            ...
            if (replied.groupedId) {
                ...
                try {
                    ...
                    const grouped = (history || []).filter(m => m && m.groupedId && String(m.groupedId) === String(replied.groupedId) && m.media);
                    if (grouped.length > 0) {
            ...
            try {
                ...
                for (const r of resultLinks) {
                    ...
                    linkParts.push(`<a href="https://t.me/addstickers/${escapeHtml(r.shortName)}">Vol.${escapeHtml(String(r.pack))} (${label})</a>`);
                }
                const linksHtml = linkParts.slice(0, 5).join(' • ');
                await editStatus(message, `✅ <b>Kang Berhasil!</b>\n${escapeHtml(String(successCount))} stiker ditambahkan ke pack Anda.\n\n👉 ${linksHtml}`);
            }
            catch (err) {
                Logger.logUser(telegramId, `Error in stickers plugin (kang): ${errMsg(err)}`, 'ERROR');
                await editStatus(message, `❌ <b>Terjadi kesalahan saat kang:</b>\n<i>${escapeHtml(errMsg(err))}</i>`);
            }
            return;
        }
        // ------------------------- .q (multi-quote) -------------------------
        ...
        if (qMatch) {
            ...
            try {
                ...
                await editStatus(message, `❌ <b>Gagal membuat quote:</b>\n<i>${escapeHtml(errMsg(err))}</i>`);
            }
        }
    }
};
