...
async function downloadToTemp(client, replied, filename) {
...
// ============================================================
async function handleZip(client, message, telegramId) {
    ...
    const rawName = (typeof replied.file?.name === 'string' && replied.file.name)
        ? replied.file.name
        : 'file';
    ...
    try {
        ...
        if (hasZipBin) {
            ...
            if (!r.ok) {
                throw new Error(`zip gagal${r.stderr ? `: ${r.stderr}` : ''}`);
            }
        }
        else if (hasPython) {
            const r = await run(PYTHON_BIN, ['-m', 'zipfile', '-c', outPath, tmpFile]);
            if (!r.ok) {
                throw new Error(`zipfile gagal${r.stderr ? `: ${r.stderr}` : ''}`);
            }
        }
        else {
...
// ============================================================
async function handleUnzip(client, message, telegramId) {
    const replied = await getReplied(message);
    ...
    const fname = (typeof replied.file?.name === 'string' && replied.file.name)
    ...
    try {
        ...
        if (hasUnzipBin) {
            ...
            if (!r.ok) {
                throw new Error(`unzip gagal${r.stderr ? `: ${r.stderr}` : ''}`);
            }
        }
        else if (hasPython) {
...
// ============================================================
async function handleDoZip(client, message, arg, telegramId) {
