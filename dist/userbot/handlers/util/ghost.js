...
const ghostGlobal = globalThis;
const ghostStore = ghostGlobal.__deltaGhostStore ?? new Map();
...
async function markRead(client, chatId) {
    try {
        ...
        Logger.logSystem(`ghost: gagal ReadHistory chat ${chatId}: ${err instanceof Error ? err.message : String(err)}`, 'WARN');
    }
}
export default {
    name: 'ghost',
    ...
    help: {
        ...
        usage: '• `.ghost` — toggle ghost di chat ini\n' +
        ...
        detail: 'Mark-as-read memakai raw API messages.ReadHistory (maxId 0) sehingga tercermin instan di sisi lawan bicara, ' +
    ...
    async execute(client, message, _settings, telegramId) {
        ...
        if (!message.out) {
            if (!ghostStore.get(idNum)?.has(chatKey)) {
                return;
            }
