...
async function resolveTargets(client, message, args) {
    const targets = [];
...
async function handleInvite(client, message, chat, isChannel, args) {
    ...
    const parts = [];
    if (ok.length > 0) {
        parts.push(`➕ Berhasil: ${ok.length}`);
    }
    if (failed.length > 0) {
        parts.push(`⚠️ Gagal: ${failed.length}`);
    }
