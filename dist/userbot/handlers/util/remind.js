...
export default {
    ...
    help: {
        ...
        usage: '• `.remind <durasi> <pesan>` — buat reminder, contoh: `.remind 1h30m beli susu`\n' +
    ...
    async execute(client, message, _settings, telegramId) {
        ...
        if (!['.remind', '.listremind', '.delremind'].includes(cmd)) {
            return;
        }
        ...
        if (!remindStore.has(idNum)) {
            remindStore.set(idNum, []);
        }
        const myReminders = remindStore.get(idNum);
        // ============ 1. .remind <durasi> <pesan> ============
        if (cmd === '.remind') {
            const remindMatch = text.match(/^\.remind\s+(\S+)\s+([\s\S]+)$/i);
            if (!remindMatch) {
                await message.edit({
                    text: `<blockquote>❌ <b>Format salah:</b> <code>.remind &lt;durasi&gt; &lt;pesan&gt;</code>\n` +
            ...
            if (delayMs > MAX_DURATION_MS) {
                await message.edit({
                    text: `<blockquote>❌ <b>Terlalu lama:</b> durasi maksimal reminder adalah <b>7 hari</b>.\n` +
        ...
        if (cmd === '.delremind') {
            ...
            if (isNaN(num) || num < 1 || num > myReminders.length) {
                await message.edit({
                    text: `<blockquote>❌ <b>Nomor tidak valid.</b> Pilih nomor 1-${myReminders.length} dari daftar <code>.listremind</code>.\n` +
            ...
            await message.edit({
                text: `🗑️ <b>Reminder Dibatalkan</b>\n\n` +
