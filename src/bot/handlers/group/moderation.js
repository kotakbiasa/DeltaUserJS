...
async function botHasPermission(ctx, perm) {
  try {
    ...
    if (me.status === 'administrator') {
      if (perm === 'ban') return me.can_restrict_members;
...
export function registerModerationHandlers(bot) {
  ...
  bot.command('ban', modCheck, async (ctx) => {
    ...
    if (!targetId) return ctx.reply('❌ Balas pesan pengguna atau ketikkan ID untuk di-ban.');
    if (!(await botHasPermission(ctx, 'ban'))) return ctx.reply('❌ Bot tidak memiliki hak untuk memblokir pengguna.');
  ...
  bot.command('unban', modCheck, async (ctx) => {
    const targetId = getTarget(ctx);
    if (!targetId) return ctx.reply('❌ Balas pesan pengguna atau ketikkan ID untuk di-unban.');
    if (!(await botHasPermission(ctx, 'ban'))) return ctx.reply('❌ Bot tidak memiliki hak admin.');
  ...
  bot.command('kick', modCheck, async (ctx) => {
    const targetId = getTarget(ctx);
    if (!targetId) return ctx.reply('❌ Balas pesan pengguna atau ketikkan ID untuk di-kick.');
    if (!(await botHasPermission(ctx, 'ban'))) return ctx.reply('❌ Bot tidak memiliki hak admin.');
  ...
  bot.command('mute', modCheck, async (ctx) => {
    const targetId = getTarget(ctx);
    if (!targetId) return ctx.reply('❌ Balas pesan pengguna untuk di-mute.');
    if (!(await botHasPermission(ctx, 'ban'))) return ctx.reply('❌ Bot tidak memiliki hak admin.');
  ...
  bot.command('unmute', modCheck, async (ctx) => {
    const targetId = getTarget(ctx);
    if (!targetId) return ctx.reply('❌ Balas pesan pengguna untuk di-unmute.');
    if (!(await botHasPermission(ctx, 'ban'))) return ctx.reply('❌ Bot tidak memiliki hak admin.');
  ...
  bot.command('pin', modCheck, async (ctx) => {
    ...
    if (!(await botHasPermission(ctx, 'pin'))) return ctx.reply('❌ Bot tidak memiliki hak pin.');
