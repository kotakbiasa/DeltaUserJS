...
export function registerZombiesHandlers(bot) {
  ...
  bot.command('zombies', modCheck, async (ctx) => {
    ...
    await ctx.reply(
      '🧟‍♂️ **Info Pemburu Zombie**\n\n' +
      'Sistem Bot Telegram resmi (Master Bot) tidak diberikan izin oleh Telegram untuk mengambil seluruh daftar member grup.\n\n' +
      'Untuk menyapu bersih akun "Deleted Account", **gunakan perintah `.zombies` dari akun Userbot kamu**. Userbot memiliki jalur akses khusus (MTProto) yang mampu mendeteksi dan menendang ribuan zombie sekaligus! 🚀',
      { parse_mode: 'Markdown' }
    );
  });
}
