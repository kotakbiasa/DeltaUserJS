...
async function sendHelpRich(ctx, richHtml, classicHtml, keyboard, deleteOld = false) {
  if (ctx.inlineMessageId) {
    ...
    await ctx.api.editMessageTextInline(ctx.inlineMessageId, classicHtml, { parse_mode: 'HTML', reply_markup: keyboard }).catch(() => { /* ignore */ });
  } else {
    await replyRich(ctx, richHtml, { reply_markup: keyboard });
    if (deleteOld) {
      try { await ctx.deleteMessage(); } catch (_) { /* message may already be gone */ }
    }
  }
}
...
export function registerInlineHelpHandlers(bot) {
  ...
  bot.callbackQuery(/^help:page:(\d+)(?::(.+))?$/, async (ctx) => {
    ...
    await sendHelpRich(
      ctx,
      buildHelpMenuRichHtml(session, page, target),
      buildHelpMenuClassicHtml(session, page, target),
      helpKeyboard(page, target, isInline),
      true
    );
  });
  ...
  bot.callbackQuery(/^help:module:([^:]+)(?::(.+))?$/, async (ctx) => {
    ...
    await sendHelpRich(
      ctx,
      buildModuleRichHtml(moduleName, session, target),
      buildModuleClassicHtml(moduleName, session, target),
      backKeyboard,
      true
    );
  });
