...
import { getUserbotSession, updateUserbotFeature, deleteUserbot, getAllRegisteredUsers, getDisabledPlugins, enablePlugin, disablePlugin, updateUserbotStatus } from '../../infrastructure/database.js';
...
export const ubotSettingsMenu = new Menu<any>('ubot-settings-menu')
  .text(
    (ctx) => {
      const session = getUserbotSession(ctx.from.id);
...
export const dangerDeleteMenu = new Menu<any>('danger-delete-menu')
...
// --- PLUGIN MANAGER MENU ---
export const pluginManagerMenu = new Menu<any>('plugin-manager-menu')
  .dynamic((ctx, range) => {
    const telegramId = ctx.from.id;
...
export const ubotMainMenu = new Menu<any>('ubot-main-menu')
  .text(
    (ctx) => {
      const isRunning = userbotManager.isRunning(ctx.from.id);
  ...
  .text('📦 Modules', async (ctx) => {
    ...
    const { buildHelpMenuRichHtml, helpKeyboard } = await import('../handlers/core/help.js');
...
// --- REGISTRATION MENU ---
export const registrationMenu = new Menu<any>('reg-menu')
...
// --- ADMIN PREMIUM MENU ---
export const adminPremiumMenu = new Menu<any>('admin-premium-menu')
  .dynamic((ctx, range) => {
...
export const adminManageUserMenu = new Menu<any>('admin-manage-user-menu')
  .dynamic((ctx, range) => {
    const targetId = ctx.session?.selectedUserToManage;
    if (!targetId) {
...
export const adminUserListMenu = new Menu<any>('admin-user-list-menu')
  .dynamic((ctx, range) => {
    const users = getAllRegisteredUsers();
    if (users.length === 0) {
...
// --- ADMIN MAIN MENU ---
export const adminMainMenu = new Menu<any>('admin-main-menu')
...
// --- MASTER MAIN MENU ---
export const masterMainMenu = new Menu<any>('master-main-menu')
  .dynamic((ctx, range) => {
...
import { inlineHelpMenu } from '../handlers/core/help.js';
