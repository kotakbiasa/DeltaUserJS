...
import type { MediaService, MediaMetadata, DownloadResult } from "./base.js";
 ...
 */
export function getService(url: string): MediaService | undefined {
 ...
 */
export async function getMetadata(url: string): Promise<MediaMetadata> {
  const service = getService(url);
  if (!service) throw new Error("No service found for this URL.");
 ...
 * Throws if no service supports the URL.
 */
export async function downloadMedia(url: string, id: string): Promise<DownloadResult> {
  const service = getService(url);
  if (!service) throw new Error("No service found for this URL.");
...
export type { MediaService, MediaMetadata, DownloadResult } from "./base.js";
