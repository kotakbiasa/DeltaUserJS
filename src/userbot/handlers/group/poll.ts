...
function parsePollArgs(args: string): ParsedPoll | { error: string } {
  let quiz = false;
...
function buildInputMediaPoll(parsed: ParsedPoll): Api.InputMediaPoll {
  ...
  const correctAnswers: unknown[] = parsed.quiz && parsed.correctIndex !== undefined
