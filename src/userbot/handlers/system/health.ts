 ...
 */
async function probeDatabase(): Promise<{ state: string; name: string; latencyMs: number | null } | null> {
  try {
...
function renderDbLine(probe: { state: string; name: string; latencyMs: number | null } | null): string {
  if (!probe) {return '';} // skip bagian DB
  let icon = '❌';
  let extra = '';
...
function renderMonitorLine(telegramId: number): string {
...
function stopMonitor(telegramId: number): boolean {
  const st = monitorStore.get(telegramId);
  if (!st) {return false;}
...
export default {
  ...
  execute: async (client, message, settings, telegramId) => {
    ...
    if (!healthMatch && !monitorMatch) {return;}
