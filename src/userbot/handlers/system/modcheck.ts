...
// Scan dist/userbot/handlers/**/*.js (file yang di-load userbot),
// hash MD5 per file, simpan snapshot/baseline di globalThis, dan
// diff-kan untuk mendeteksi modul curiga yang di-drop hot.
// Tanpa dependency eksternal: crypto.createHash + fs.
// ─────────────────────────────────────────────────────────────
...
const SUSPICIOUS_PATTERNS: Array<{ tag: string; re: RegExp }> = [
...
function getFromGlobal(key: string): Snapshot | null {
  const store = globalThis as unknown as Record<string, unknown>;
  const raw = store[key];
  if (raw && typeof raw === 'object' && (raw as Snapshot).files) {return raw as Snapshot;}
  return null;
}

function setToGlobal(key: string, snap: Snapshot): void {
  (globalThis as unknown as Record<string, unknown>)[key] = snap;
}
...
function detectFlags(buf: Buffer): string[] {
...
async function scanPlugins(): Promise<Snapshot> {
...
function diffAgainstBaseline(current: Snapshot, baseline: Snapshot): DiffRow[] {
  const rows: DiffRow[] = [];
...
function renderJson(payload: { files: unknown[]; summary: string }): string {
...
function jsonSection(payload: { files: unknown[]; summary: string }): string {
...
const STATUS_EMOJI: Record<DiffRow['status'], string> = {
  'BARU': '🟢',
  'DIHAPUS': '🔴',
  'DIUBAH': '🟡',
};
