...
// Perintah:
//   .font <teks>             (daftar 12 gaya + preview tiap gaya)
//   .font <teks> | <gaya>    (langsung kirim versi gaya itu)
// Semua map unicode ditulis manual per gaya (A-Z a-z 0-9).
// Gaya matematika (bold, italic, script, fraktur, dst.) punya
// beberapa code point yang TIDAK dipakai Unicode ( réservé), jadi
// huruf tsb dipetakan eksplisit ke karakter resmi padanannya.
// ============================================================
...
const FONT_STYLES: Record<string, FontStyleMap> = {
  'bold': {
...
function applyFont(text: string, style: FontStyleMap): string {
...
function resolveStyleName(raw: string): string | null {
...
export default {
  ...
  help: {
    ...
    usage:
      '• `.font <teks>` — lihat daftar 12 gaya + preview tiap gaya\n' +
      '• `.font <teks> | <gaya>` — langsung kirim versi gaya itu, contoh: `.font Halo | bold`',
    detail:
      'Gaya tersedia: ' + STYLE_NAMES.join(', ') + '. ' +
  ...
  async execute(client, message, _settings, _telegramId) {
    ...
    if (!arg) {
      await message.edit({
        text:
          `<blockquote>❌ <b>Format salah:</b> <code>.font &lt;teks&gt;</code>\n` +
