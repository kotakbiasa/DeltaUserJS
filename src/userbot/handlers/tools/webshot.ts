...
const isImageBuffer = (buf: Buffer): boolean => {
...
async function shotThumio(url: string): Promise<Buffer> {
  ...
  try {
    ...
    if (!isImageBuffer(buf)) {throw new Error('thum.io respons bukan gambar');}
    return buf;
  } finally {
    t.done();
  }
...
async function shotMshots(url: string): Promise<Buffer> {
  const t = withTimeout(FETCH_TIMEOUT);
  try {
    ...
    if (!res.ok) {throw new Error(`mshots HTTP ${res.status}`);}
    const buf = Buffer.from(await res.arrayBuffer());
    if (!isImageBuffer(buf)) {throw new Error('mshots respons bukan gambar');}
    return buf;
  } finally {
    t.done();
  }
...
async function shotMicrolink(url: string): Promise<Buffer> {
  const t = withTimeout(FETCH_TIMEOUT);
  try {
    ...
    const buf = Buffer.from(await res.arrayBuffer());
    if (!isImageBuffer(buf)) {throw new Error('microlink respons bukan gambar');}
    return buf;
  } finally {
    t.done();
  }
...
const PROVIDERS: Array<[string, (url: string) => Promise<Buffer>]> = [
  ['thum.io', shotThumio],
  ['mshots', shotMshots],
  ['microlink', shotMicrolink],
];

async function getShot(url: string): Promise<ShotResult> {
  const errors: string[] = [];
...
async function resolveUrl(message: any, arg: string): Promise<string | null> {
  ...
  const replyText: string = typeof replied?.message === 'string' ? replied.message : '';
  if (replyText) {
