...
export default {
  ...
  async execute(client, message, _settings, telegramId) {
    ...
    if (!word) {
      await message.edit({
        text:
          `<blockquote>❌ <b>Format salah:</b> <code>.define &lt;kata&gt;</code>\n` +
    ...
    try {
      ...
      if (Array.isArray(entry.phonetics)) {
        for (const p of entry.phonetics) {
          if (!phonetic && p && typeof p.text === 'string' && p.text) {phonetic = p.text;}
          if (!audio && p && typeof p.audio === 'string' && p.audio) {audio = p.audio;}
        }
      }
      ...
      for (const m of meanings) {
        ...
        const top = defs[0];
        if (!top || !top.definition) {continue;}
      ...
      const head =
        `📖 <b>${escapeHtml(entry.word || word)}</b>` +
