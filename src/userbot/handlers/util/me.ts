 ...
 *
 * .setinfo <teks> → set kartu profil (bio singkat, max 500 karakter)
 * .resetinfo      → hapus kartu profil
 * .me             → tampilkan kartu: foto profil + nama + username + custom info.
 *                   Kalau PP ada → kirim photo dengan caption.
 *                   Kalau tidak  → text blockquote.
 *                   Sambil reply ke user lain → lihat kartu dia.
 *
 * Store disimpan di globalThis (bukan murni module-level) supaya bertahan
 * saat plugin hot-reload (pluginLoader cache-bust & re-import modul ini).
 * Key: telegramId (number), value: kartu.
 */
...
const globalStore = globalThis as unknown as { [STORE_KEY]?: Map<number, ProfileCard> };
