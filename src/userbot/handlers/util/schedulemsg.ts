...
interface ScheduleEntry {
  ...
  text: string;         // teks asli pesan (verbatim)
  media: unknown;       // media pesan asli (bila ada), null bila teks saja
...
const scheduleStore = new Map<number, ScheduleEntry[]>();
...
function formatTarget(epochMs: number): string {
...
function contentKind(entry: { text: string; media: unknown }): string {
  if (entry.media) {return entry.text ? 'media + caption' : 'media';}
  return 'teks';
}
...
function startScheduleTimer(client, idNum: number, entry: ScheduleEntry, delayMs: number) {
  const timeoutId = setTimeout(() => {
    ...
    if (list) {
      ...
      if (list.length === 0) {scheduleStore.delete(idNum);}
    }
...
export default {
  ...
  help: {
    ...
    usage:
      '• `.schedule <durasi>` (reply pesan yang mau dikirim ulang) — contoh: reply lalu `.schedule 1h30m`\n' +
  ...
  async execute(client, message, _settings, telegramId) {
    ...
    if (!['.schedule', '.listschedule', '.delschedule'].includes(cmd)) {return;}

    const idNum = Number(telegramId);
    const chatId = message.chatId;
    if (chatId === null || chatId === undefined) {return;}

    if (!scheduleStore.has(idNum)) {scheduleStore.set(idNum, []);}
    const mySchedules = scheduleStore.get(idNum) as ScheduleEntry[];
    ...
    if (cmd === '.schedule') {
      ...
      if (!argMatch) {
        await message.edit({
          text:
            `<blockquote>❌ <b>Format salah:</b> reply pesan yang mau dikirim ulang, lalu ketik <code>.schedule &lt;durasi&gt;</code>\n` +
      ...
      if (delayMs > MAX_DURATION_MS) {
        await message.edit({
          text:
            `<blockquote>❌ <b>Terlalu lama:</b> durasi maksimal jadwal adalah <b>7 hari</b>.\n` +
    ...
    if (cmd === '.delschedule') {
      ...
      if (Number.isNaN(num) || num < 1 || num > mySchedules.length) {
        await message.edit({
          text:
            `<blockquote>❌ <b>Nomor tidak valid.</b> Pilih nomor 1-${mySchedules.length} dari daftar <code>.listschedule</code>.\n` +
      ...
      if (mySchedules.length === 0) {scheduleStore.delete(idNum);}

      await message.edit({
        text:
          `🗑️ <b>Pesan Terjadwal Dibatalkan</b>\n\n` +
