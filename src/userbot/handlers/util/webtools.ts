...
const clamp = (s: string, n: number): string => (s.length > n ? `${s.slice(0, n - 1).trimEnd()}…` : s);
...
const buildDns = async (host: string): Promise<string> => {
  ...
  try {
    ...
    if (!res.ok) {throw new Error(`dns.google menjawab HTTP ${res.status}`);}
    const data = asObj(await res.json());
    const status = asNum(data.Status);
    if (status === 3) {throw new Error('Domain tidak ditemukan (NXDOMAIN)');}
    if (status !== 0) {throw new Error(`DNS error status ${status}`);}
    ...
    if (aRecords.length === 0 && chain.length === 0) {throw new Error('Tidak ada record A untuk domain ini');}
    ...
    if (aRecords.length > 0) {
      ...
      if (aRecords.length > 8) {body += `  - <i>+${aRecords.length - 8} lainnya</i>\n`;}
    } else {
    ...
    if (ttl > 0) {body += `• <b>TTL</b>: <code>${ttl}s</code>\n`;}
...
const buildPing = async (host: string): Promise<string> => {
  ...
  try {
    ...
    let icon = '❔';
    if (res.status >= 200 && res.status < 300) {icon = '✅';}
    else if (res.status >= 300 && res.status < 400) {icon = '↪️';}
    else if (res.status >= 400 && res.status < 500) {icon = '⚠️';}
    else if (res.status >= 500) {icon = '⛔';}
    ...
    if (res.redirected && res.url) {body += `• <b>Redirect</b>: <code>${escapeHtml(res.url)}</code>\n`;}
    return `🏓 <b>Ping Web</b>\n\n<blockquote>${body.trimEnd()}</blockquote>`;
  } finally {
    t.done();
  }
...
const buildWhois = async (host: string): Promise<string> => {
  ...
  try {
    ...
    if (res.status === 404) {throw new Error('Domain tidak ditemukan di registry (404)');}
    if (!res.ok) {throw new Error(`rdap.org menjawab HTTP ${res.status}`);}
    const data = asObj(await res.json());

    let registrar = '';
    for (const e of asArr(data.entities)) {
      const ent = asObj(e);
      if (!asArr(ent.roles).map(String).includes('registrar')) {continue;}
    ...
    const eventDate = (action: string): string => {
    ...
    if (registrar) {body += `• <b>Registrar</b>: <code>${escapeHtml(registrar)}</code>\n`;}
    const created = eventDate('registration');
    const updated = eventDate('last changed');
    const expires = eventDate('expiration');
    if (created) {body += `• <b>Dibuat</b>: <code>${escapeHtml(created)}</code>\n`;}
    if (expires) {body += `• <b>Expired</b>: <code>${escapeHtml(expires)}</code>\n`;}
    if (updated) {body += `• <b>Diperbarui</b>: <code>${escapeHtml(updated)}</code>\n`;}
    if (statusList.length > 0) {body += `• <b>Status</b>: <code>${escapeHtml(statusList.slice(0, 4).join(', '))}</code>\n`;}
    if (typeof dnssec === 'boolean') {body += `• <b>DNSSEC</b>: ${dnssec ? 'Aktif' : 'Tidak'}\n`;}
...
const buildUd = async (term: string): Promise<string> => {
  const t = withTimeout(15000);
  try {
    ...
    if (!res.ok) {throw new Error(`Urban Dictionary menjawab HTTP ${res.status}`);}
    const data = asObj(await res.json());
    const list = asArr(data.list);
    if (list.length === 0) {throw new Error(`Tidak ada definisi untuk "${term}"`);}
...
const spcheckViaXon = async (email: string): Promise<string> => {
  const t = withTimeout(20000);
  try {
    ...
    if (!res.ok) {throw new Error(`XposedOrNot menjawab HTTP ${res.status}`);}
    const data = asObj(await res.json());
    ...
    if (sites.length === 0) {
      ...
      if (records > 0) {body += `• <b>Total rekaman terekspos</b>: <code>${records.toLocaleString('en-US')}</code>\n`;}
    }
    if (pastes > 0) {body += `• <b>Pastes</b>: <code>${pastes}</code>\n`;}
...
const spcheckViaLeakcheck = async (email: string): Promise<string> => {
  const t = withTimeout(20000);
  try {
    ...
    if (!res.ok) {throw new Error(`LeakCheck menjawab HTTP ${res.status}`);}
    const data = asObj(await res.json());
    if (data.success !== true) {throw new Error('LeakCheck menjawab success=false');}
    ...
    if (found === 0) {
      ...
      if (sources.length > 0) {
        ...
        if (sources.length > 8) {body += `  - <i>+${sources.length - 8} lainnya</i>\n`;}
      }
    }
...
const buildSpcheck = async (email: string): Promise<string> => {
  try {
