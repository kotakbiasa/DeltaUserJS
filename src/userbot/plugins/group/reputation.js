import { getChatSettings, updateChatSettings, getReputation, updateReputation, getUserVar } from '../../../core/database.js';
  ...
  async execute(client, message, settings, telegramId) {
    ...
    if (message.out && message.message) {
      ...
      if (cmd === '.setrepfloor') {
        ...
        const name = userEntity.firstName || userEntity.username || `User_${targetId}`;
        await message.edit({
        ...
        for (const entry of list) {
          ...
          const name = userEntity.firstName || userEntity.username || `User_${entry.userId}`;
    ...
    if (isUpvote || isDownvote) {
      ...
      const targetName = targetEntity.firstName || targetEntity.username || `User_${targetId}`;
      const voterEntity = await client.getEntity(senderId);
      const voterName = voterEntity.firstName || voterEntity.username || `User_${senderId}`;
